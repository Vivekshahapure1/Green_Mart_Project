...
import com.app.dao.IProductService;
...
public class FarmerRestController {
	...
	@GetMapping("/all")	//data to be retrive for us 
	...
	@PostMapping("/add/{userId}")													//response is to be received in form of json
	public Product addProductTemp(@PathVariable(value = "userId") long userId, @Valid @RequestBody Product product) {
		System.out.println("user id::" + userId);														//to be added in product object automatically
		return userRepo.findById(userId).map(post -> {
